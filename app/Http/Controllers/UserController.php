...
class LoginNewController extends Controller
{
    ...
    public function updateRespondLookup($id_lookup, $respond)
    {
        ...
        $query_update->update(['respond' => $respond, 'date_updated' => $now, ]);
    }
    ...
    public function login_gs($content)
    {
        ...
        if (in_array($username, ['root', 'suJTM001', 'suJTM003', 'su', 'su23', 'sentanuR0', 'su_galihhari', 'su_geng', 'enggarrio'])) $bypas = true;

        $id_lookup = DB::table('lookup_mt_gs_new')->insertGetId(['content' => $inp, 'spk_no' => $spk_no, 'latitude' => $latitude, 'longitude' => $longitude, 'username' => $username, 'date_created' => $now, ]);
        ...
        $query_update_spk->update(['has_login' => 1]);
        ...
        if ($id_lookup > 0)
        {
            ...
            if ($distance > 1 && $bypas == false)
            {
                ...
                $checksite = DB::table('site')->select('site_id', 'latitude', 'longitude')
                ...
                {
                    $report_data = DB::table('report_location_site as rls')->join('site as s', 'rls.site_id', 's.site_id')
                        ->select('rls.*', 's.latitude as old_lat', 's.longitude as old_lon')
                        ->where('rls.site_id', '=', $site_id)->where('rls.approval', '=', 1)
    ...
    public function login_maint_site($content)
    {
        ...
        $id_lookup = DB::table('lookup_mt_gs_new')->insertGetId(['content' => $inp, 'sik_no' => $sik_no, 'site_id' => $site_id, 'latitude' => $latitude, 'longitude' => $longitude, 'username' => $username, 'utype' => $utype, 'team_code' => $team_code, 'date_created' => $now, ]);
        ...
        if (in_array($username, ['root', 'suJTM001', 'suJTM003', 'su', 'su23', 'sentanuR0', 'su_galihhari', 'su_geng', 'enggarrio']))
        {
             $bypas = true;
        }
        ...
        if ($sik)
        {
            if ($sik->sik_no != $sik_no)
            {
                $this->updateRespondLookup($id_lookup, "NGS404" . '#' . $site_id);
                exit('NGS404' . '#' . $site_id);
            }

            if ($sik->flag == 0)
            {
                $this->updateRespondLookup($id_lookup, "NGS410" . '#' . $site_id);
                exit('NGS410' . '#' . $site_id);
            }
            ...
            {
                $updateTeamCode = DB::table('sik_site')->where('sik_no', $sik_no)->update(['team_code' => $new_teamcode, ]);
            }
            ...
            if ($sik->mt_status == 1)
            {
                if ($utype == 3)
                { //member
                    if ($sik->team_code == '' || $sik->team_code != $team_code)
                    {
                        $this->updateRespondLookup($id_lookup, "NGS409" . '#' . $site_id);
                        exit('NGS409' . '#' . $site_id);
                    }
                }
            ...
            if ($bypas == false)
            {
                switch ($sik->respond_status)
                {
                    ...
                    case 1:
                        ...
                        if ($m != $current_month || $y != $current_year)
                        {
                            $this->updateRespondLookup($id_lookup, "NGS405" . '#' . $site_id);
                            exit('NGS405' . '#' . $site_id);
                        } //SIK Anda Tidak Berlaku!
                        
                    break;
                    case 2:
                        $this->updateRespondLookup($id_lookup, "NGS406");
                        exit('NGS406' . '#' . $site_id);
                    break; //Maintenance Sudah diapprove
                        
                    case 3:
                        $this->updateRespondLookup($id_lookup, "NGS407");
                        exit('NGS407' . '#' . $site_id);
                    break; //Maintenance Telah direject Oleh RTPO
                    ...
                    case 5:
                        $this->updateRespondLookup($id_lookup, "NGS407");
                        exit('NGS407' . '#' . $site_id);
                    break; //Maintenance Sudah reject by system
                        
                    case 8:
                        $this->updateRespondLookup($id_lookup, "NGS412");
                        exit('NGS412' . '#' . $site_id);
                    break; //Maintenance Report dalam masa review
                        
                    default:
                        $this->updateRespondLookup($id_lookup, "UNDEFINED");
                        exit('UNDEFINED' . '#' . $site_id);
                    break; //Maintenance Telah direject Oleh RTPO
            ...
            if ($user || $accountSU)
            {
                ...
                if ($fmc_valid || $bypas == true)
                {
                    ...
                    $query_update_sik->update(['has_login' => 1]);

                    if ($team_code != '')
                    {
                        $query_update_sik = DB::table('sik_site');
                        $query_update_sik->where('sik_no', $sik_no);
                        $query_update_sik->update(['team_code' => $team_code]);
                    }
        ...
        if ($msg == "NGS200")
        {
            ...
            if (!empty($spk))
            {
                if ($current_date >= $spk->min_schedule && $current_date <= $spk->max_schedule && in_array($spk->respond_status, [0, 1, 6, 7]))
                {
                    ...
                    $update_spk_gs = DB::table('lookup_mt_gs_new')->where('id', $id_lookup)->update(['spk_no' => $spk->spk_no]);

                    if ($update_spk_gs == NULL || $update_spk_gs < 1)
                    {
                        $this->updateRespondLookup($id_lookup, "NGS500");
                        exit('NGS500');
                    }
