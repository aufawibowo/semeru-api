...
class CreateInvoiceService
{
    ...
    public function execute(CreateInvoiceRequest $request)
    {
        ...
        $shippingAddress = $this->profileRepository->getAddressById($request->shippingAddress);
        ...
        if(count($checkProductStock) > 0)
            throw new InvalidOperationException("Stok produk tidak mencukupi " . json_encode($checkProductStock));
        ...
        if(!$save)
        {
            throw new InvalidOperationException('Gagal membuat invoice.');
        }
        else{
            return $save;
        }
