...
class GetOrderService
{
    ...
    public function execute(GetOrderRequest $request)
    {
        ...
        if(!$order){
            throw new InvalidOperationException('Order tidak ditemukan');
        }
        else{
            $order[0]['products'][0]['photo_url'] = $this->productPhotoService->transformPath(
                $order[0]['products'][0]['photo_url'],
                $order[0]['products'][0]['id']
            );
            return $order[0];
        }
    }
