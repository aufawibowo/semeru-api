...
class SqlInvoiceRepository implements InvoiceRepository
{
    ...
    public function save(Invoice $invoice, Order $order): bool
    {
        ...
        if(count($order->getProducts()) < 1)
        {
            ...
            {
                if($this->addToCart($order)){
                    $order->setCart([$params['id']]);
                }
                else{
                    return false;
                }
        ...
        $sql = "select c.id, c.product_id, p.stock, c.qty, p.price, p.seller_id
            from cart c
            inner join products p on p.id = c.product_id
            where c.id in "
            .
            $this->whereInBuilder($order->getCart())
            .
            "and c.deleted_at is null
            and c.is_checked_out != 1
            and p.deleted_at is null";

        $orderProducts = $this->db->fetchAll($sql, PDO::FETCH_ASSOC);
        ...
        $orderIds = [];
        $amountTotal = 0;
        $sqlOrderProducts = "insert into 
                                order_products (id, order_id, product_id, quantity, amount, cart_id) 
                            values ";
        ...
        $sqlOrders = "insert into 
                        orders (id, invoice_id, seller_id, customer_id, courier_id, status, expired_at) 
                        values ";
    ...
    private function addToCart(Order $order): bool
    {
