...
class SqlOrderRepository implements OrderRepository
{
    ...
    public function get(OrderId $orderId, string $customerId)
    {
        ...
        foreach ($orders as $key => $value) {
            ...
            $result = $this->db->fetchAll($sql, PDO::FETCH_ASSOC, ['order_id' => $orderId->id()]);

            $orders[$key]['products'] = $result;
            ...
            foreach ($orders[$key]['products'] as $key => $value){
                ...
                $orders[$key]['is_reviewed'] = $this->isOrderReviewed($orderId->id(), $customerId, $value['id']);
            }
    ...
    private function isOrderReviewed(string $orderId, string $customerId, string $productId)
    {
        $sql = "select *
                from reviews
                inner join orders o on reviews.order_id = o.id
                inner join customers c on o.customer_id = c.user_id
                inner join products p on reviews.product_id = p.id
                where 
                    o.id = :order_id
                    and
                    c.user_id = :customer_id
        ...
        if(!$this->db->fetchAll($sql, PDO::FETCH_ASSOC, $params)){
            return false;
        }
        else{
            return true;
