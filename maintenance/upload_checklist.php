...
// exit($username);
if ($maintenance == 0 || $username=='root') {
			if (isset($_FILES['ceklist']['name'])) {

				// exit('file exist');
				$tmp_count = 0;
				$success_count = 0;
				foreach ($_FILES['ceklist']['name'] as $tmp_count => $name) {  

					// if($tmp_count>1) continue;
					$file_upload_url = $tmp_landing;
					...
					try {
						...
						// else continue;
							...
							if ($check_photo) {
								...
								}elseif (strpos( $checklist_cat, 'GS' )==4) {
